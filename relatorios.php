...
$periodo = $_GET['periodo'];
    ...
    // 1. Definição de colunas (correta, com o tipo 'status')
    $colunas = [
        ...
        ['campo' => 'horario', 'titulo' => 'Horário'],
        ['campo' => 'sala_alocada', 'titulo' => 'Sala Alocada'],
        ['campo' => 'status_final', 'titulo' => 'Status', 'tipo' => 'status'],
        ['campo' => 'detalhes_conflito', 'titulo' => 'Detalhes do Conflito']
    ];

    // 2. Consulta principal CORRIGIDA para buscar todos os campos necessários
    $sql_todas_as_turmas = "
        SELECT 
            t.id, t.codigo, t.nome, t.professor, t.turno,
            t.horario_inicio, t.horario_fim,
            t.segunda, t.terca, t.quarta, t.quinta, t.sexta, t.sabado,
            t.sala_fixa_id,
            s.codigo as sala_fixa_codigo
        FROM turmas t
    ...
    // 3. Lógica de detecção de conflitos (comparando turmas diretamente)
    $conflitos_mapeados = [];
    for ($i = 0; $i < count($todas_as_turmas); $i++) {
        for ($j = $i + 1; $j < count($todas_as_turmas); $j++) {
            $t1 = $todas_as_turmas[$i];
            $t2 = $todas_as_turmas[$j];
    ...
    // 4. Construir as linhas de resultado final, combinando com os dados do ensalamento
    $linhas = [];
    foreach ($todas_as_turmas as $turma) {
        // Busca os dados de ensalamento para esta turma específica
        $sql_ensalamentos = "
            SELECT e.dia_semana, e.status, s.codigo as sala_codigo
        ...
        // Se o status lido do ensalamento for conflito, mas nossa lógica não encontrou sobreposição, é por falta de sala.
        ...
        $linhas[] = [
            ...
            'detalhes_conflito' => implode('; ', $detalhes)
        ];
    }
